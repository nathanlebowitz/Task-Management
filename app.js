...
async function addTaskBtnClicked() {
  ...
  //add the task object to the task array
  let newTaskObject = {
...
async function taskCompleted(btnCompleteelement) {
  let id = btnCompleteelement.parentNode.parentNode.getAttribute('data-task-id');
...
function buildTaskElement(task, index) {
  ...
  taskElement.taskIndex = index;
  taskElement.setAttribute('data-task-id', index);



  taskElement.innerHTML = `<p>${task.title + '. Added By: ' + task.userName} </p>
    <div>
    ${task.done ? "" :
